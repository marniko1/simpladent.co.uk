...
jQuery(document).scroll(function(){
	if( jQuery('.picturepopup_div').length == 1 )
	{
		jQuery(document).scrollTop(scollingtop);
	}
	else
	{
...
jQuery('a.picturepopup').click(function(e){
	jQuery('<div class="picturepopup_div"><center><div class="picturepopup_picture_box"><div class="picturepopup_picture_close">x</div></div></center></div>').prependTo('body');
	jQuery('.picturepopup_picture_box').append( '<img src="'+jQuery(this).attr('href')+'" />' );
	scollingtop=jQuery(document).scrollTop();

	jQuery('.picturepopup_div').on('click', function(){
		jQuery('.picturepopup_div').remove();
	});
    ...
    map:{
        ...
        initialisation:function(){
            ...
            $(that.citiesList).find(that.citiesListItem).each(function(index, element){
				...
				var clinic_id = cityLink.data('clinic');
                var coords = that.coordsTransform(cityLink.data('coords'));
                var cityPoint = $('<div id="'+clinic_id+'" class="' + that.cityClass + '" title="' + cityLink.html() + '" ></div>').css(coords);
...
// *************************Small resolution navigation functionalities*************************
jQuery(document).ready(function(){

    let nav, ul;
    // hide/show menu
    $('.lr_menu_button').on('click', function(){
    ...
    // hide/show submenu
    $('.lr_menu .hasSubMenu').on('click', function(){
        ul = $(this).find('ul');
    ...
    $('.lr_menu .hasSubMenu .subMenu .sub_menu a').on('click', function(){
        $(nav).addClass('d-none');
        $(ul).addClass('d-none');
    });
})
